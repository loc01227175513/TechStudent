...
export default function Page() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await Axios.get(`http://127.0.0.1:8000/api/Khoahocchitiet/${id}`);
        ...
        if (event.data === window.YT.PlayerState.ENDED) {
            ...
            if (parsedData && currentVideoId) {
                try {
                    ...
                    const response = await Axios.post('http://127.0.0.1:8000/api/videodahoc', requestData);
    ...
    useEffect(() => {
        const checkVideoWatched = async () => {
            ...
            try {
                const response = await Axios.get(`http://127.0.0.1:8000/api/kiemtravidedahoc`);
